        ...
        <div className="max-w-[750px] w-full">
          ...
          <section className="bg-grey-10 rounded w-full py-9 px-7 mb-9">
            ...
            <div className="md:flex items-center justify-between">
              <div>
                <span className="bg-brand-4 text-brand-1 rounded font-inter font-medium text-sm py-1 px-2 mr-3">{announce.year}</span>
                <span className="bg-brand-4 text-brand-1 rounded font-inter font-medium text-sm py-1 px-2">0 KM</span>
              </div>
              <small className="text-grey-01 text-base font-medium block mt-8 md:mt-0">{formatPrice(announce.price)}</small>
            </div>
              ...
              {announce.images.map((image, index) => (
                <li key={index}>
                  <figure className="bg-grey-8 rounded w-[5rem] h-[5rem] grid place-items-center overflow-hidden">
                    <Image src={image.imageUrl} alt="Imagem do Carro" className="w-full h-auto" width={70} height={50} />
                  </figure>
                </li>
              ))}
            </ul>
